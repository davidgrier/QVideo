'''Unit tests for QIDSCamera and QIDSSource.'''
import sys
import unittest
import numpy as np
...
# QIDSTree is imported after that happens it fails to subclass QGenicamTree.
sys.modules.setdefault('QVideo.cameras.IDS.QIDSTree', MagicMock())

from QVideo.cameras.IDS.QIDSCamera import QIDSCamera, QIDSSource

_cam_module = sys.modules['QVideo.cameras.Genicam.QGenicamCamera']
_MODULE = sys.modules['QVideo.cameras.IDS.QIDSCamera']
_ICategory = _cam_module.ICategory
_PRODUCER = '/fake/ids_u3vgentl.cti'
...
def _make_device():
    ...
    device.remote_device.node_map.DeviceModelName.value = 'TestIDS'
...
def make_camera(cameraID=0):
    ...
    with patch.object(QIDSCamera, 'producer', _PRODUCER):
        with patch.object(_cam_module, 'Harvester', return_value=harvester):
            cam = QIDSCamera(cameraID=cameraID)
...
class TestAll(unittest.TestCase):
    ...
    def test_all_contains_camera(self):
        self.assertIn('QIDSCamera', _MODULE.__all__)

    def test_all_contains_source(self):
        self.assertIn('QIDSSource', _MODULE.__all__)


class TestSubclass(unittest.TestCase):

    def test_is_genicam_subclass(self):
        self.assertTrue(issubclass(QIDSCamera, QGenicamCamera))
...
class TestSource(unittest.TestCase):
    ...
    def test_source_creates_camera_when_none(self):
        ...
        with patch.object(QIDSCamera, 'producer', _PRODUCER):
            with patch.object(_cam_module, 'Harvester', return_value=harvester):
                src = QIDSSource()
        self.assertIsInstance(src.source, QIDSCamera)
        src.source.close()
