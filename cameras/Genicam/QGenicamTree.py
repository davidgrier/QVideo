...
class QGenicamTree(QCameraTree):
    ...
    def __init__(self, *args,
        ...
        description = self.description(camera.node('Root'))
        self.visibility = visibility
        super().__init__(camera, description['children'], *args, **kwargs)
        self.controls = controls
        self._updateEnabled()

    def description(self, feature: IValue) -> dict:
        ...
        if isinstance(feature, ICategory):
            this['type'] = 'group'
            this['children'] = [self.description(f)
    ...
    def controls(self, controls: list[str] | None) -> None:
        ...
        for item in self.listAllItems()[1:]:
            ...
            if (controls is None) or (name in controls):
                visibility = self.camera.node(name).visibility
            else:
    ...
    def _updateVisible(self) -> None:
        for item in self.listAllItems()[1:]:
            p = item.param
            visibility = p.opts.get('visibility', EVisibility.Invisible)
