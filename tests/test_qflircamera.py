'''Unit tests for QFlirCamera and QFlirSource.'''
import sys
import unittest
import numpy as np
...
from QVideo.cameras.Flir.QFlirCamera import QFlirCamera, QFlirSource

_cam_module = sys.modules['QVideo.cameras.Genicam.QGenicamCamera']
_MODULE = sys.modules['QVideo.cameras.Flir.QFlirCamera']
_ICategory = _cam_module.ICategory
_PRODUCER = '/fake/Spinnaker_GenTL.cti'
...
def _make_device():
    ...
    device.remote_device.node_map.DeviceModelName.value = 'TestFlir'
...
def make_camera(cameraID=0):
    ...
    with patch.object(QFlirCamera, 'producer', _PRODUCER):
        with patch.object(_cam_module, 'Harvester', return_value=harvester):
            cam = QFlirCamera(cameraID=cameraID)
...
class TestAll(unittest.TestCase):
    ...
    def test_all_contains_camera(self):
        self.assertIn('QFlirCamera', _MODULE.__all__)

    def test_all_contains_source(self):
        self.assertIn('QFlirSource', _MODULE.__all__)


class TestSubclass(unittest.TestCase):

    def test_is_genicam_subclass(self):
        self.assertTrue(issubclass(QFlirCamera, QGenicamCamera))
...
class TestSource(unittest.TestCase):
    ...
    def test_source_creates_camera_when_none(self):
        ...
        with patch.object(QFlirCamera, 'producer', _PRODUCER):
            with patch.object(_cam_module, 'Harvester', return_value=harvester):
                src = QFlirSource()
        self.assertIsInstance(src.source, QFlirCamera)
        src.source.close()
