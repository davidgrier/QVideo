'''Unit tests for demos.uniformitydemo.'''
import unittest
...
from QVideo.demos.uniformitydemo import UniformityDemo


app = QtWidgets.QApplication.instance() or QtWidgets.QApplication([])


def make_demo() -> UniformityDemo:
    return UniformityDemo(QNoiseTree())


class TestUniformityDemo(unittest.TestCase):

    def setUp(self):
        self.widget = make_demo()

    def test_creates_successfully(self):
        self.assertIsInstance(self.widget, UniformityDemo)
    ...
    def test_uniformity_attribute_is_quniformitywidget(self):
        self.assertIsInstance(self.widget.uniformity, QUniformityWidget)

    def test_uniformity_connected_to_screen(self):
        self.assertIs(self.widget.uniformity.screen, self.widget.screen)
    ...
    def test_controls_contains_uniformity(self):
        items = [self.widget._controls.itemAt(i).widget()
                 for i in range(self.widget._controls.count())]
        self.assertIn(self.widget.uniformity, items)

    def test_screen_source_connected_to_tree_source(self):
        tree = QNoiseTree()
        widget = UniformityDemo(tree)
